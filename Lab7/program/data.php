...
class Data
{
    public function get_data($link)
    {
        ...
        $full_date_pattern = '/<div id="infoTime">Оновлено (\d{2}.\d{2}.\d{4})/';
        $full_date = $this->find_one_value($full_date_pattern, $out);
        ...
        $current_day_pattern = '/<p class="date ".*?>(.*?)<\/p>/s';
        ...
        $data = $this->make_array_of_values($city_name, $sunrise_time, $sunset_time, $date, $temperature, $today_weather, $today_temperature, $current_day, $current_week_day, $current_month, $arr_of_weather, $first_time, $current_hour, $full_date);
        return $data;
    }

    private function make_array_of_values($city_name, $sunrise_time, $sunset_time, $date, $temperature, $today_weather, $today_temperature, $current_day, $current_week_day, $current_month, $arr_of_weather, $first_time, $current_hour, $full_date)
    {
        $data = [];
    ...
    private function convert_to_array($str)
    {
        ...
        foreach ($arr as $val) {
            if (!$this->check_if_contains($val)) {
                ...
                $val = $this->remove_unnecessary_things($val);
                $val = $this->remove_plus_if_necessary($val);
                array_push($new_arr, $val);
            }
        }
        $new_arr = $this->remove_wrong_elements($new_arr);
        return $new_arr;
    }

    private function remove_unnecessary_things($string)
    {
        $index = strpos($string, "&");        
    ...
    private function remove_wrong_elements($arr){
        $new_arr = [];
        foreach($arr as $val){
            if ($val != 'td' && $val != ''){
                array_push($new_arr, (int)$val);
            }
        }
        return $new_arr;
    }
